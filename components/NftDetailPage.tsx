...
interface NftDetailProps {
  ...
  onPurchase: (nftData: NftData, seller: User) => void;
  onBack: () => void;
}

const NftDetailPage: React.FC<NftDetailProps> = ({ nftData, currentUser, owner, creator, onUpdate, onPurchase, onBack }) => {
  ...
  const handleConfirmPurchase = () => {
    if (!nftData.price || currentUser.walletBalance < nftData.price) {
  ...
  const isOwnedByCurrentUser = nftData.ownerId === currentUser.id;
  const isForSale = nftData.status === 'for_sale';
  const formattedPrice = nftData.price ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'ETH' }).format(nftData.price) : 'N/A';
  const canAfford = nftData.price ? currentUser.walletBalance >= nftData.price : false;
  ...
  return (
    <div className="animate-fade-in-up">
        ...
        {showPurchaseModal && <PurchaseModal onConfirm={handleConfirmPurchase} onCancel={() => setShowPurchaseModal(false)} price={formattedPrice} nftData={nftData} canAfford={canAfford} />}
            ...
            <div className="lg:w-1/2">
                <div className="bg-brand-surface/50 backdrop-blur-sm rounded-2xl shadow-2xl shadow-brand-primary/10 border border-white/10 p-6">
                    <div className="relative mb-6 group">
                        <img src={nftData.imageUrl!} alt="Launched NFT" className={`rounded-xl w-full transition-all duration-500 ${isOwnedByCurrentUser ? '' : 'blur-sm'}`} />
                        {!isOwnedByCurrentUser && isForSale && (
                    ...
                    <div className="text-left space-y-4">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-sm text-gray-400">Created by {creator.username}</p>
                                <p className="text-sm text-gray-400">Owned by {owner.username}</p>
                            </div>
            ...
            <div className="lg:w-1/2">
                <div className="bg-brand-surface/50 backdrop-blur-sm rounded-2xl shadow-2xl shadow-brand-primary/10 border border-white/10 p-6 space-y-6">
                    ...
                    <div>
                        ...
                        {isOwnedByCurrentUser ? (
                            <div className="space-y-3">
                                ...
                                { isForSale ? (
                                    <div className="text-center p-3 bg-green-900/50 rounded-lg text-green-300">Currently listed for sale.</div>
                                ) : (
                    ...
                    <div>
                        ...
                        <ul className="space-y-3 max-h-[22rem] overflow-y-auto pr-2">
                            {nftData.priceHistory.slice().reverse().map((entry, index) => {
                                ...
                                const priceChange = prevPrice && entry.type !== 'sale' ? entry.price - prevPrice : 0;

                                return(
                                    <li key={index} className="text-sm p-3 bg-brand-background/50 rounded-md">
                                        ...
                                        {entry.type === 'sale' && (
                                            <div className="text-xs text-gray-400 mt-2 pt-2 border-t border-white/10 space-y-1">
                                                <p><strong>Seller Proceeds:</strong> <span className="font-mono text-green-400">+Ξ {entry.sellerProceeds?.toFixed(4)}</span></p>
                                                <p><strong>Platform Fee (30%):</strong> <span className="font-mono text-red-400">-Ξ {entry.platformFee?.toFixed(4)}</span></p>
                                            </div>
                                        )}
                                    </li>
                                )
